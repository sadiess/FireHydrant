        ...
        if ":leaf=" in line:
            ...
            if not fillna is None:
                ...
                if "nan" in line:
                    line = line.replace("nan", "-999")
            result = re.match(
                r"(\t*)(\d+):leaf=({0})$".format(regex_float_pattern), line
            )
            if not result:
                print(line)
...
def convert_model(model, input_variables, output_xml, fillna=None):
    ...
    Info_Creator = ET.SubElement(
        GeneralInfo, "Info", name="Creator", value="xgboost2TMVA"
    )
    ...
    Weights = ET.SubElement(
        MethodSetup, "Weights", NTrees=str(NTrees), AnalysisType="1"
    )

    for itree in range(NTrees):
        BinaryTree = ET.SubElement(
            Weights,
            "BinaryTree",
            type="DecisionTree",
            boostWeight="1.0e+00",
